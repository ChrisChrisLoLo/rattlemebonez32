...
translate([0, 0, 0])rotate([0,0,0])mirror([0,0,0]){
...
//      difference(){
        ...
        skin([for (i=[0:layers]) transform(translation(CapTranslation(i, keyID+1)) * rotation(CapRotation(i, keyID+1)), elliptical_rectangle([13.5,13.5], b =  [1,1],fn=fn))]); //outer shell
        }else {
...
module gasket(){
    ...
    linear_extrude(height = gasket_height) {
        difference() {
            ...
            union() {
                square([10.7,5.3], center = true);
                translate([0,(5.3/2)+(2/2),0])
